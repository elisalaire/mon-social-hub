...
function App() {
  ...
  useEffect(() => {
    ...
    fetchEvents();
    const channel = supabase.channel('ultimate-stability-v16').on('postgres_changes', { event: '*', schema: 'public', table: 'events' }, () => fetchEvents()).subscribe();
  ...
  const handleSaveEvent = async () => {
    ...
    if (editingEventId) {
      const { data: updatedData } = await supabase.from('events').update(data).eq('id', editingEventId).select();
      if (updatedData) setActiveEvent(updatedData[0]); // Update UI card immediately
    } else {
  ...
  return (
    <div className="h-screen bg-[#0b0118] text-white font-sans flex flex-col overflow-hidden">
      ...
      <header className="p-3 md:px-10 flex justify-between items-center border-b border-white/10 bg-[#0b0118]/80 backdrop-blur-xl z-50 shrink-0">
        ...
        <div onClick={() => setIsEditingProfile(true)} className="flex items-center gap-3 bg-white/5 p-1 pr-4 rounded-full cursor-pointer hover:border-[#FF2E95] border border-white/10 transition-all">
      ...
      <main className="flex-1 flex flex-col p-2 md:p-6 overflow-hidden">
        <div className="flex items-center justify-between mb-4 bg-white/5 p-4 rounded-3xl border border-white/10 shrink-0 shadow-xl">
          <button onClick={() => setCurrentMonth(subMonths(currentMonth, 1))} className="text-[#00F0FF] text-3xl font-black px-6 hover:scale-125 transition-all">◀</button>
          <h2 className="text-xl md:text-4xl font-black uppercase italic text-[#D1FF4B] tracking-widest">{format(currentMonth, 'MMMM yyyy')}</h2>
          <button onClick={() => setCurrentMonth(addMonths(currentMonth, 1))} className="text-[#00F0FF] text-3xl font-black px-6 hover:scale-125 transition-all">▶</button>
        </div>

        <div className="flex-1 grid grid-cols-7 gap-2 rounded-3xl overflow-hidden border border-white/10 bg-white/[0.02]">
          ...
          {eachDayOfInterval({ 
            ...
            return (
              <div key={i} className={`relative min-h-0 p-4 border border-white/5 transition-all flex flex-col gap-3 ${isCurrent ? 'bg-white/5 hover:bg-white/10 cursor-pointer' : 'opacity-10 pointer-events-none'} ${isToday ? 'bg-[#D1FF4B]/10 ring-2 ring-[#D1FF4B]/40' : ''}`} onClick={() => isCurrent && (dayEvents.length > 0 ? setActiveEvent(dayEvents[0]) : setSelectedDay(day))}>
                ...
                <div className="flex-1 overflow-y-auto space-y-2 custom-scrollbar">
                  {dayEvents.map(e => (
                    <div key={e.id} onClick={(ev) => { ev.stopPropagation(); setActiveEvent(e); }} 
                      className={`p-3 rounded-2xl text-[10px] font-black uppercase shadow-lg border border-white/10 ${e.is_public ? 'bg-[#00F0FF] text-black' : e.hype > 15 ? 'bg-[#D1FF4B] text-black shadow-[0_0_15px_rgba(209,255,75,0.4)]' : 'bg-[#FF2E95] text-white'}`}>
      ...
      {/* EVENT OVERLAY - NO SCROLL SIDEBAR */}
      {activeEvent && (
          ...
          <div className="bg-[#1a0b2e] border-2 border-[#D1FF4B] w-full max-w-6xl h-[90vh] rounded-[50px] overflow-hidden flex flex-col md:flex-row shadow-2xl relative" onClick={e => e.stopPropagation()}>
            ...
            <div className="md:w-[45%] flex flex-col bg-[#0e021f] h-full border-r border-white/10 overflow-hidden">
              <div className="relative h-64 shrink-0">
                ...
                <div className="absolute bottom-6 left-8 right-8">
                  {activeEvent.is_public && <span className="bg-[#00F0FF] text-black text-[9px] font-black px-4 py-1.5 rounded-full uppercase mb-3 inline-block tracking-widest shadow-lg">Public</span>}
                  <h3 className="text-4xl font-black uppercase tracking-tighter text-white leading-none truncate">{activeEvent.title}</h3>
                  <div className="flex flex-wrap gap-1 mt-2">
                    {activeEvent.tags?.split(',').map((tag, idx) => <span key={idx} className="text-[8px] font-black bg-white/10 px-2 py-0.5 rounded-full uppercase text-[#D1FF4B]">#{tag.trim()}</span>)}
                  </div>
                </div>
              </div>
              ...
              <div className="flex-1 p-8 flex flex-col justify-between overflow-hidden">
                <div className="space-y-6">
                  <div className="grid grid-cols-2 gap-2">
                    {activeEvent.ticket_url && <a href={activeEvent.ticket_url} target="_blank" className="bg-[#00F0FF] text-black text-center py-4 rounded-3xl font-black uppercase tracking-widest text-xs">Tickets 🎫</a>}
                    {activeEvent.event_url && <a href={activeEvent.event_url} target="_blank" className="bg-white/10 text-white text-center py-4 rounded-3xl font-black uppercase text-xs">Event Info 🔗</a>}
                  </div>
                  <div className="grid grid-cols-3 gap-3">
                    ...
                    <button onClick={() => { const start = format(new Date(activeEvent.date), "yyyyMMdd"); window.open(`https://calendar.google.com/calendar/render?action=TEMPLATE&text=${activeEvent.title}&dates=${start}/${start}`, '_blank') }} className="bg-white/5 p-4 rounded-3xl border border-white/5 flex items-center justify-center hover:bg-[#D1FF4B] hover:text-black transition-all font-black text-[10px]">📅 CAL</button>
                  </div>
                  <p className="text-white/60 font-medium leading-relaxed italic text-lg line-clamp-2">"{activeEvent.description || "No description provided."}"</p>
                  <div className="bg-white/5 p-4 rounded-3xl border border-white/10"><p className="text-sm font-bold truncate">📍 {activeEvent.location}</p></div>
                  
                  <div>
                    <p className="text-[10px] font-black uppercase opacity-40 mb-3 tracking-widest">Crew ({activeEvent.attendees?.length || 0})</p>
                    <div className="flex flex-wrap gap-3 max-h-32 overflow-y-auto custom-scrollbar">
                      {activeEvent.attendees?.map((a, i) => (
                        <div key={i} className="group relative">
                          <div className={`p-0.5 rounded-full ${a.has_ticket ? 'bg-[#00F0FF]' : 'bg-transparent'}`}>
                             <img src={a.avatar} className="w-12 h-12 rounded-full border-2 border-[#0e021f]" />
                          </div>
                          <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-2 bg-[#D1FF4B] text-black rounded-xl text-[10px] font-black uppercase whitespace-nowrap opacity-0 group-hover:opacity-100 transition-all pointer-events-none z-50">
                ...
                <div className="flex gap-4 pt-6 mt-auto">
                  {activeEvent.attendees?.some(a => a.name === user.name) ? (
                    <>
                      ...
                      <button onClick={() => handleJoin(activeEvent)} className="bg-white/5 border border-white/10 px-8 py-5 rounded-[25px] font-black uppercase text-xs opacity-50 hover:text-red-500 transition-all">Leave</button>
                    </>
                  ) : <button onClick={() => handleJoin(activeEvent)} className="flex-1 bg-[#FF2E95] text-white py-6 rounded-[25px] font-black uppercase tracking-widest shadow-xl">Join the Vibe</button>}
                </div>
              </div>
            </div>
            {/* Chat Side */}
            <div className="flex-1 flex flex-col bg-[#0b0118] h-full overflow-hidden">
              ...
              <div className="p-8 bg-white/5 border-t border-white/5 flex gap-3 shrink-0">
                ...
                <button onClick={() => postComment(activeEvent)} className="bg-[#D1FF4B] text-black font-black px-8 py-4 rounded-2xl uppercase text-[10px] tracking-widest shadow-lg active:scale-90 transition-all">Send</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* CREATE / EDIT MODAL */}
      {selectedDay && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[300] flex items-center justify-center p-4" onClick={() => { setSelectedDay(null); setEditingEventId(null); setSelectedGif(null); }}>
          <div className="bg-[#1a0b2e] border-2 border-[#D1FF4B] p-10 rounded-[50px] w-full max-w-2xl max-h-[90vh] overflow-y-auto custom-scrollbar" onClick={e => e.stopPropagation()}>
             <h3 className="text-3xl font-black mb-8 text-[#D1FF4B] uppercase italic text-center">{editingEventId ? "✏️ Edit Vibe" : "🚀 New Vibe"}</h3>
             <div className="space-y-4 mb-8">
               ...
               <div className="grid grid-cols-2 gap-4 bg-white/5 p-4 rounded-3xl border border-white/10 text-center">
                  <div><p className="text-[10px] font-black uppercase text-[#00F0FF] mb-2">Hour</p><input type="time" className="bg-transparent font-black w-full text-center outline-none text-white text-xl" value={form.time} onChange={e => setForm({...form, time: e.target.value})} /></div>
                  <div className="flex flex-col justify-center"><p className="text-[10px] font-black uppercase text-[#00F0FF] mb-1">Date</p><p className="font-black text-lg text-[#D1FF4B]">{format(selectedDay, 'dd MMM')}</p></div>
               </div>
             ...
             <button onClick={handleSaveEvent} className="w-full bg-[#D1FF4B] text-black font-black py-6 rounded-[30px] uppercase text-xl tracking-widest shadow-neon active:scale-95 transition-all">{editingEventId ? "Update Vibe" : "Launch Vibe"}</button>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/95 z-[400] flex items-center justify-center p-4" onClick={() => setIsEditingProfile(false)}>
           <div className="bg-[#1a0b2e] border-2 border-[#FF2E95] p-10 rounded-[60px] w-full max-w-sm shadow-2xl" onClick={e => e.stopPropagation()}>
              <div className="flex flex-col items-center gap-8 text-center">
                ...
                <input className="input-premium text-center font-black uppercase text-xl border-[#FF2E95]/20 focus:border-[#FF2E95]" value={user.name} onChange={e => setUser({...user, name: e.target.value, avatar: `https://api.dicebear.com/7.x/adventurer/svg?seed=${e.target.value}`})} placeholder="Username" />
                <textarea className="input-premium h-24 resize-none text-center border-[#FF2E95]/20 focus:border-[#FF2E95]" value={user.bio} onChange={e => setUser({...user, bio: e.target.value})} placeholder="Status" />
                <button onClick={() => setIsEditingProfile(false)} className="w-full bg-[#FF2E95] text-white font-black py-6 rounded-[30px] uppercase tracking-widest active:scale-95 transition-all">Update</button>
              </div>
           </div>
        </div>
      )}

      <style>{`.shadow-neon { box-shadow: 0 0 30px rgba(209,255,75,0.2); } .custom-scrollbar::-webkit-scrollbar { width: 4px; } .custom-scrollbar::-webkit-scrollbar-thumb { background: #D1FF4B; border-radius: 10px; } .input-premium { background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.1); border-radius: 25px; padding: 20px; font-weight: bold; color: white; width: 100%; outline: none; transition: all 0.3s; } .input-premium:focus { border-color: #D1FF4B; background: rgba(255,255,255,0.06); }`}</style>
    </div>
  )
}
export default App
