...
function App() {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('final-stable-v7')
      ...
      <main className="flex-1 flex flex-col p-4 md:p-6 overflow-hidden bg-gradient-to-b from-[#0b0118] to-[#120428]">
        ...
        <div className="flex-1 grid grid-cols-7 gap-2 rounded-3xl overflow-hidden border border-white/10">
          ...
          {eachDayOfInterval({ 
            ...
            return (
              <div key={i} className={`relative min-h-0 p-4 border border-white/5 flex flex-col gap-3 ${isCurrent ? 'bg-white/5 hover:bg-white/10 cursor-pointer' : 'opacity-10 pointer-events-none'} ${isToday ? 'bg-[#D1FF4B]/10 ring-2 ring-[#D1FF4B]/40' : ''}`} onClick={() => isCurrent && dayEvents.length === 0 && setSelectedDay(day)}>
                ...
                <div className="flex-1 overflow-y-auto space-y-2 custom-scrollbar">
                  {dayEvents.map(e => (
                    <div key={e.id} onClick={(ev) => { ev.stopPropagation(); setActiveEvent(e); }} 
                      className={`p-3 rounded-2xl text-[10px] font-black uppercase shadow-lg transition-all border border-white/10 ${e.ticket_url ? 'bg-[#00F0FF] text-black shadow-[0_0_15px_rgba(0,240,255,0.4)]' : e.hype > 15 ? 'bg-[#D1FF4B] text-black shadow-[0_0_15px_rgba(209,255,75,0.4)]' : 'bg-[#FF2E95] text-white'}`}>
                      <div className="truncate mb-2 pr-1">{e.ticket_url ? '🎫 ' : ''}{e.title}</div>
          ...
          <div className="bg-[#1a0b2e] border-2 border-[#D1FF4B] w-full max-w-6xl h-[90vh] rounded-[50px] overflow-hidden flex flex-col md:flex-row shadow-2xl relative" onClick={e => e.stopPropagation()}>
            ...
            <div className="md:w-[45%] flex flex-col bg-[#0e021f] h-full border-r border-white/10">
              <div className="relative h-64 shrink-0">
                ...
                <div className="absolute bottom-6 left-8 right-8">
                  {activeEvent.ticket_url && <span className="bg-[#00F0FF] text-black text-[9px] font-black px-4 py-1.5 rounded-full uppercase mb-3 inline-block tracking-widest shadow-lg animate-pulse">Ticketed Event</span>}
              ...
              <div className="flex-1 p-8 flex flex-col justify-between overflow-hidden">
                <div className="space-y-6">
                  {activeEvent.ticket_url && (
                      <a href={activeEvent.ticket_url} target="_blank" rel="noreferrer" className="block w-full bg-[#00F0FF] text-black text-center py-5 rounded-[25px] font-black uppercase tracking-widest text-sm shadow-xl hover:scale-[1.02] transition-all">Buy Tickets Now 🎫</a>
                  )}
                  ...
                  <p className="text-white/60 font-medium leading-relaxed italic text-lg line-clamp-3">"{activeEvent.description || "No description provided."}"</p>
                  
                  <div className="bg-white/5 p-4 rounded-3xl border border-white/10"><p className="text-[8px] font-black uppercase opacity-40 text-[#00F0FF] mb-1">Location</p><p className="text-sm font-bold truncate">📍 {activeEvent.location}</p></div>

                  <div>
                    <p className="text-[10px] font-black uppercase opacity-40 mb-3 tracking-widest">Crew List ({activeEvent.attendees?.length || 0})</p>
                    <div className="flex flex-wrap gap-3 max-h-32 overflow-y-auto custom-scrollbar pr-2">
                      {activeEvent.attendees?.map((a, i) => (
                        <div key={i} className="group relative">
                          ...
                          <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-2 bg-[#D1FF4B] text-black rounded-xl text-[10px] font-black uppercase opacity-0 group-hover:opacity-100 transition-all pointer-events-none z-50 shadow-2xl">
            ...
            <div className="flex-1 flex flex-col bg-[#0b0118] h-full overflow-hidden">
              ...
              <div className="p-8 bg-white/5 border-t border-white/5 flex gap-3 shrink-0">
                <input className="bg-transparent flex-1 font-bold outline-none text-white text-base" placeholder="Send a vibe..." value={commentInput} onChange={e => setCommentInput(e.target.value)} onKeyPress={ev => ev.key === 'Enter' && postComment(activeEvent)} />
      ...
      {selectedDay && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[300] flex items-center justify-center p-4" onClick={() => setSelectedDay(null)}>
          <div className="bg-[#1a0b2e] border-2 border-[#D1FF4B] p-10 rounded-[50px] w-full max-w-xl max-h-[90vh] overflow-y-auto custom-scrollbar" onClick={e => e.stopPropagation()}>
             <h3 className="text-3xl font-black mb-8 text-[#D1FF4B] uppercase italic text-center tracking-tighter">Broadcast New Vibe</h3>
             <div className="space-y-4 mb-10">
               ...
               <div className="grid grid-cols-2 gap-4">
                  <div className="bg-white/5 p-5 rounded-2xl border border-white/10 text-center"><p className="text-[10px] font-black uppercase text-[#00F0FF] mb-2 tracking-widest">Hour</p><input type="time" className="bg-transparent font-black w-full text-center outline-none text-white text-xl" value={form.time} onChange={e => setForm({...form, time: e.target.value})} /></div>
               ...
               <div className="bg-white/5 p-6 rounded-[35px] border border-white/10">
                 <p className="text-[10px] font-black uppercase text-[#00F0FF] mb-4 text-center tracking-widest">Visual Vibe Search</p>
