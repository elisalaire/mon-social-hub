...
function App() {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('final-v10')
    ...
    <div className="h-screen bg-[#0b0118] text-white font-sans flex flex-col overflow-hidden">
      ...
      <main className="flex-1 flex flex-col p-4 md:p-6 overflow-hidden bg-gradient-to-b from-[#0b0118] to-[#120428]">
        ...
        <div className="flex-1 grid grid-cols-7 gap-2 rounded-3xl overflow-hidden border border-white/10">
          {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map(d => <div key={d} className="bg-white/5 p-4 text-center text-[12px] font-black opacity-30 uppercase tracking-[0.3em] border-b border-white/5">{d}</div>)}
          {eachDayOfInterval({ 
                ...
                <div className="flex-1 overflow-y-auto space-y-2 custom-scrollbar">
                  {dayEvents.map(e => (
                    <div key={e.id} onClick={(ev) => { ev.stopPropagation(); setActiveEvent(e); }} 
                      className={`p-3 rounded-2xl text-[10px] font-black uppercase shadow-lg transition-all border border-white/10 ${e.ticket_url ? 'bg-[#00F0FF] text-black shadow-[0_0_15px_rgba(0,240,255,0.4)]' : e.hype > 15 ? 'bg-[#D1FF4B] text-black' : 'bg-[#FF2E95] text-white'}`}>
                      <div className="truncate mb-2">{e.ticket_url ? '🎫 ' : ''}{e.title}</div>
                      <div className="flex -space-x-2">
                        {e.attendees?.slice(0, 4).map((a, idx) => <img key={idx} src={a.avatar} className="w-5 h-5 rounded-full border border-black/20 shadow-md" />)}
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <div className="bg-[#1a0b2e] border-2 border-[#D1FF4B] w-full max-w-6xl h-[90vh] rounded-[50px] overflow-hidden flex flex-col md:flex-row shadow-2xl relative" onClick={e => e.stopPropagation()}>
            ...
            <div className="md:w-[45%] flex flex-col bg-[#0e021f] h-full border-r border-white/10">
              <div className="relative h-64 shrink-0">
                ...
                <div className="absolute top-6 right-6 flex gap-2">
                   {activeEvent.attendees?.[0]?.name === user.name && (
                     <>
                        <button onClick={() => { setEditingEventId(activeEvent.id); setForm(activeEvent); setSelectedDay(new Date(activeEvent.date)); setSelectedGif(activeEvent.gif_url); setActiveEvent(null); }} className="bg-white/10 p-3 rounded-full hover:bg-[#D1FF4B] hover:text-black transition-all">✏️</button>
                        <button onClick={() => deleteEvent(activeEvent.id)} className="bg-red-500/20 p-3 rounded-full hover:bg-red-500 text-white transition-all">🗑️</button>
                     </>
                   )}
                </div>
                <div className="absolute bottom-6 left-8 right-8">
                  {activeEvent.ticket_url && <span className="bg-[#00F0FF] text-black text-[9px] font-black px-4 py-1.5 rounded-full uppercase mb-3 inline-block tracking-widest shadow-lg">Public</span>}
              ...
              <div className="flex-1 p-8 flex flex-col justify-between overflow-hidden">
                <div className="space-y-6">
                  {activeEvent.ticket_url && <a href={activeEvent.ticket_url} target="_blank" className="block w-full bg-[#00F0FF] text-black text-center py-5 rounded-[25px] font-black uppercase tracking-widest text-sm shadow-xl">Tickets 🎫</a>}
                  ...
                  <p className="text-white/60 leading-relaxed font-medium italic text-lg line-clamp-2">"{activeEvent.description || "No description provided."}"</p>
                  <div className="p-4 bg-white/5 rounded-3xl border border-white/10"><p className="text-[8px] font-black uppercase opacity-40 text-[#00F0FF] mb-1">Location</p><p className="text-sm font-bold truncate">📍 {activeEvent.location}</p></div>
                      ...
                      {activeEvent.attendees?.map((a, i) => (
                        <div key={i} className="group relative">
                          <div className={`p-0.5 rounded-full ${a.has_ticket ? 'bg-[#00F0FF]' : 'bg-transparent'}`}>
                             <img src={a.avatar} className="w-12 h-12 rounded-full border-2 border-[#0e021f]" alt={a.name} />
                          </div>
                          <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 px-3 py-2 bg-[#D1FF4B] text-black rounded-xl text-[10px] font-black uppercase opacity-0 group-hover:opacity-100 transition-all pointer-events-none z-50 shadow-xl">
                  ...
                  {activeEvent.attendees?.some(a => a.name === user.name) ? (
                    <>
                      <button onClick={() => toggleTicketStatus(activeEvent)} className={`flex-1 py-5 rounded-[25px] font-black uppercase text-xs tracking-widest border-2 transition-all ${activeEvent.attendees.find(a => a.name === user.name)?.has_ticket ? 'bg-[#00F0FF] border-[#00F0FF] text-black' : 'border-[#00F0FF] text-[#00F0FF] hover:bg-[#00F0FF]/10'}`}>
                        {activeEvent.attendees.find(a => a.name === user.name)?.has_ticket ? 'Ticket Confirmed ✓' : 'Confirm My Ticket'}
                      </button>
            ...
            <div className="flex-1 flex flex-col bg-[#0b0118] h-full overflow-hidden">
                ...
                {(activeEvent.comments || []).map((c, i) => (
                  <div key={i} className={`flex flex-col ${c.user === user.name ? 'items-end' : 'items-start'}`}>
                    ...
                    <div className="flex gap-4 items-end">
                      {c.user !== user.name && <img src={c.avatar} className="w-10 h-10 rounded-full border border-white/10" />}
              ...
              <div className="p-8 bg-white/5 border-t border-white/5 flex gap-3 shrink-0">
                <input className="bg-transparent flex-1 font-bold outline-none text-white text-base" placeholder="Send message..." value={commentInput} onChange={e => setCommentInput(e.target.value)} onKeyPress={ev => ev.key === 'Enter' && postComment(activeEvent)} />
                <button onClick={() => postComment(activeEvent)} className="bg-[#D1FF4B] text-black font-black px-8 py-4 rounded-2xl uppercase text-[10px] tracking-widest shadow-lg transition-all active:scale-90">Send</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* CREATE / EDIT MODAL */}
      {selectedDay && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[300] flex items-center justify-center p-4" onClick={() => { setSelectedDay(null); setEditingEventId(null); setForm({ title: "", price: "", location: "", description: "", recap_url: "", time: "20:00", ticket_url: "" }); }}>
          <div className="bg-[#1a0b2e] border-2 border-[#D1FF4B] p-10 rounded-[50px] w-full max-w-xl max-h-[90vh] overflow-y-auto custom-scrollbar" onClick={e => e.stopPropagation()}>
             <h3 className="text-3xl font-black mb-8 text-[#D1FF4B] uppercase italic text-center">{editingEventId ? "Update Vibe" : "New Vibe"}</h3>
             <div className="space-y-4 mb-8">
               ...
               <div className="grid grid-cols-2 gap-4">
                  <div className="bg-white/5 p-5 rounded-2xl border border-white/10 text-center"><p className="text-[10px] font-black uppercase text-[#00F0FF] mb-2">Hour</p><input type="time" className="bg-transparent font-black w-full text-center outline-none text-white text-xl" value={form.time} onChange={e => setForm({...form, time: e.target.value})} /></div>
               ...
               <div className="bg-white/5 p-6 rounded-[35px] border border-white/10">
                 <p className="text-[10px] font-black uppercase text-[#00F0FF] mb-4 text-center tracking-widest">GIF Vibe</p>
             ...
             <button onClick={handleSaveEvent} className="w-full bg-[#D1FF4B] text-black font-black py-6 rounded-[30px] uppercase text-xl tracking-widest shadow-neon">{editingEventId ? "Update" : "Launch"}</button>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/95 z-[400] flex items-center justify-center p-4" onClick={() => setIsEditingProfile(false)}>
           <div className="bg-[#1a0b2e] border-2 border-[#D1FF4B] p-10 rounded-[60px] w-full max-w-sm shadow-2xl" onClick={e => e.stopPropagation()}>
              <div className="flex flex-col items-center gap-8 text-center">
                ...
                <button onClick={() => setIsEditingProfile(false)} className="w-full bg-[#D1FF4B] text-black font-black py-6 rounded-[30px] uppercase tracking-widest shadow-xl">Save</button>
              </div>
           </div>
        </div>
      )}
