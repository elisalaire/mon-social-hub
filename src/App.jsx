...
function App() {
  ...
  useEffect(() => {
    ...
    fetchEvents();
    const channel = supabase.channel('v13-final').on('postgres_changes', { event: '*', schema: 'public', table: 'events' }, () => fetchEvents()).subscribe();
  ...
  return (
    <div className="h-screen bg-[#0b0118] text-white font-sans flex flex-col overflow-hidden">
      <header className="p-3 md:px-10 flex justify-between items-center border-b border-white/10 bg-[#0b0118]/80 backdrop-blur-xl z-50 shrink-0">
        ...
        <div onClick={() => setIsEditingProfile(true)} className="flex items-center gap-2 bg-white/5 p-1 pr-3 rounded-full cursor-pointer hover:bg-white/10 border border-white/10 transition-all active:scale-95">
          <img src={user.avatar} className="w-7 h-7 md:w-8 md:h-8 rounded-full border border-white/10" alt="avatar" />
          <span className="text-[10px] font-black uppercase">{user.name || "Set Identity"}</span>
        </div>
      </header>

      <main className="flex-1 flex flex-col p-2 md:p-4 overflow-hidden bg-gradient-to-b from-[#0b0118] to-[#0e021f]">
        ...
        <div className="flex-1 grid grid-cols-7 gap-1 rounded-2xl overflow-hidden border border-white/10 bg-white/[0.01]">
          ...
          {eachDayOfInterval({ start: startOfWeek(startOfMonth(currentMonth), {weekStartsOn:1}), end: endOfWeek(endOfMonth(currentMonth), {weekStartsOn:1}) }).map((day, i) => {
            ...
            return (
              <div key={i} className={`relative min-h-0 p-1 md:p-2 border-[0.5px] border-white/5 flex flex-col gap-1 transition-all ${isCurrent ? 'bg-white/5 hover:bg-white/[0.06] cursor-pointer' : 'opacity-5 pointer-events-none'} ${isToday ? 'bg-[#D1FF4B]/5 ring-1 ring-inset ring-[#D1FF4B]/30' : ''}`} onClick={() => isCurrent && (dayEvents.length > 0 ? setActiveEvent(dayEvents[0]) : setSelectedDay(day))}>
                <div className="flex justify-between items-center shrink-0">
                  <span className={`text-[11px] md:text-[14px] font-black ${isToday ? 'text-[#D1FF4B]' : 'opacity-30'}`}>{format(day, 'd')}</span>
                  {isCurrent && <button onClick={(e) => { e.stopPropagation(); setSelectedDay(day); }} className="text-[14px] opacity-20 hover:opacity-100 hover:text-[#D1FF4B]">+</button>}
                </div>
      ...
      {activeEvent && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[100] flex items-center justify-center p-0 md:p-4 animate-in fade-in duration-300" onClick={() => setActiveEvent(null)}>
          <div className="bg-[#1a0b2e] border-0 md:border-2 border-white/10 w-full h-full md:h-[85vh] md:max-w-6xl md:rounded-[40px] overflow-hidden flex flex-col md:flex-row shadow-2xl relative" onClick={e => e.stopPropagation()}>
            <div className="flex-1 md:w-[42%] flex flex-col bg-[#0e021f] h-full border-r border-white/10 overflow-hidden">
              <div className="relative h-44 md:h-52 shrink-0">
                ...
                <div className="absolute top-4 right-4 flex gap-2">
                   <button onClick={() => { setEditingEventId(activeEvent.id); setForm(activeEvent); setSelectedDay(new Date(activeEvent.date)); setSelectedGif(activeEvent.gif_url); setActiveEvent(null); }} className="bg-black/50 backdrop-blur-md p-2 rounded-xl border border-white/10 hover:bg-[#D1FF4B] hover:text-black transition-all">✏️</button>
                   <button onClick={() => { if(window.confirm("Delete?")) supabase.from('events').delete().eq('id', activeEvent.id).then(() => setActiveEvent(null)) }} className="bg-black/50 backdrop-blur-md p-2 rounded-xl border border-white/10 hover:bg-red-500 transition-all">🗑️</button>
                </div>
              ...
              <div className="p-4 md:p-6 space-y-4 overflow-hidden flex flex-col justify-between h-full">
                ...
                <div className="grid grid-cols-3 gap-2">
                  <div className="bg-white/5 p-2 rounded-xl text-center border border-white/5"><p className="text-[7px] font-black uppercase text-[#00F0FF] mb-1">Time</p><p className="font-black text-xs">{activeEvent.time}</p></div>
                  <div className="bg-white/5 p-2 rounded-xl text-center border border-white/5"><p className="text-[7px] font-black uppercase text-[#00F0FF] mb-1">Price</p><p className="font-black text-xs truncate">{activeEvent.price || 'Free'}</p></div>
                  <button onClick={() => { const start = format(new Date(activeEvent.date), "yyyyMMdd"); window.open(`https://calendar.google.com/calendar/render?action=TEMPLATE&text=${activeEvent.title}&dates=${start}/${start}`, '_blank') }} className="bg-white/5 p-2 rounded-xl border border-white/5 text-[9px] font-black hover:bg-[#D1FF4B] hover:text-black">📅 CAL</button>
                </div>
                <p className="text-white/60 text-xs leading-relaxed italic line-clamp-2">"{activeEvent.description || "No description."}"</p>
                <div className="bg-white/5 p-2 rounded-xl border border-white/5"><p className="text-xs font-bold truncate">📍 {activeEvent.location}</p></div>
                <div className="flex-1 overflow-hidden">
                  ...
                  <div className="flex flex-wrap gap-2 max-h-20 overflow-y-auto custom-scrollbar">
                    {activeEvent.attendees?.map((a, i) => (
                      <div key={i} className="group relative" onClick={(e) => { e.stopPropagation(); alert(`${a.name}: ${a.bio}`); }}>
                        <div className={`p-1 rounded-full bg-white/5 ${a.has_ticket ? 'border border-[#00F0FF]' : 'border border-transparent'}`}>
                           <img src={a.avatar} className="w-8 h-8 rounded-full" alt={a.name} />
                           {a.has_ticket && <div className="absolute -top-1 -right-1 bg-[#00F0FF] text-black text-[8px] w-4 h-4 rounded-full flex items-center justify-center font-bold border border-[#1a0b2e]">✓</div>}
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            <div className="flex-1 flex flex-col bg-[#0b0118] h-full overflow-hidden">
              <div className="p-4 border-b border-white/5 flex justify-between items-center bg-black/20 shrink-0">
                  ...
                  <button onClick={() => handleHype(activeEvent)} className="text-sm">⚡️ {activeEvent.hype || 0}</button>
              </div>
                ...
                {(activeEvent.comments || []).map((c, i) => (
                  <div key={i} className={`flex flex-col ${c.user === user.name ? 'items-end' : 'items-start'}`}>
                    ...
                    <div className="flex gap-2 items-end">
                      {c.user !== user.name && <img src={c.avatar} className="w-6 h-6 rounded-full shadow-lg" alt="u" />}
      ...
      {selectedDay && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[300] flex items-center justify-center p-2" onClick={() => setSelectedDay(null)}>
          <div className="bg-[#1a0b2e] border-2 border-[#D1FF4B] p-6 rounded-3xl w-full max-w-xl max-h-[90vh] overflow-y-auto custom-scrollbar shadow-neon" onClick={e => e.stopPropagation()}>
             <h3 className="text-xl font-black mb-6 text-[#D1FF4B] uppercase italic text-center">{editingEventId ? "✏️ Edit Vibe" : "🚀 New Vibe"}</h3>
             <div className="space-y-3">
               <div className="flex gap-2 mb-4 bg-white/5 p-1 rounded-xl border border-white/10">
                 <button onClick={() => setForm({...form, is_public: false})} className={`flex-1 py-2 rounded-lg text-[9px] font-black uppercase transition-all ${!form.is_public ? 'bg-[#FF2E95] text-white' : 'opacity-40'}`}>Private</button>
                 <button onClick={() => setForm({...form, is_public: true})} className={`flex-1 py-2 rounded-lg text-[9px] font-black uppercase transition-all ${form.is_public ? 'bg-[#00F0FF] text-black' : 'opacity-40'}`}>Public</button>
               </div>
               <input className="input-premium py-3" placeholder="Title" value={form.title} onChange={e => setForm({...form, title: e.target.value})} />
               {form.is_public && (
                 <div className="grid grid-cols-2 gap-2 animate-in slide-in-from-top-2">
                   <input className="input-premium py-3 text-[#00F0FF]" placeholder="Event Link" value={form.event_url} onChange={e => setForm({...form, event_url: e.target.value})} />
                   <input className="input-premium py-3 text-[#00F0FF]" placeholder="Ticket Link" value={form.ticket_url} onChange={e => setForm({...form, ticket_url: e.target.value})} />
                 </div>
               )}
               <div className="grid grid-cols-2 gap-2">
                 <input className="input-premium py-3" placeholder="Price" value={form.price} onChange={e => setForm({...form, price: e.target.value})} />
                 <input className="input-premium py-3" placeholder="Location" value={form.location} onChange={e => setForm({...form, location: e.target.value})} />
               </div>
               <input className="input-premium py-3" placeholder="Tags (Tech, House, Chill...)" value={form.tags} onChange={e => setForm({...form, tags: e.target.value})} />
               <div className="grid grid-cols-2 gap-2 bg-white/5 p-3 rounded-2xl border border-white/10">
                  <div className="text-center"><p className="text-[8px] font-black uppercase text-[#00F0FF]">Hour</p><input type="time" className="bg-transparent font-black w-full text-center outline-none text-white text-lg" value={form.time} onChange={e => setForm({...form, time: e.target.value})} /></div>
                  <div className="text-center flex flex-col justify-center"><p className="text-[8px] font-black uppercase text-[#00F0FF]">Date</p><p className="font-black text-sm text-[#D1FF4B]">{format(selectedDay, 'dd MMM')}</p></div>
               </div>
               <textarea className="input-premium h-20 py-3 resize-none" placeholder="Details..." value={form.description} onChange={e => setForm({...form, description: e.target.value})} />
               <input className="input-premium py-3" placeholder="Search GIF..." onChange={e => searchGiphy(e.target.value)} />
               <div className="grid grid-cols-3 gap-2">
                 {gifResults.map(g => <img key={g.id} src={g.images.fixed_height_small.url} onClick={() => setSelectedGif(g.images.fixed_height.url)} className={`h-12 w-full object-cover rounded-xl cursor-pointer border-2 transition-all ${selectedGif === g.images.fixed_height.url ? 'border-[#D1FF4B]' : 'border-transparent opacity-40'}`} />)}
               </div>
             </div>
             <button onClick={handleSaveEvent} className="w-full bg-[#D1FF4B] text-black font-black py-4 mt-6 rounded-2xl uppercase text-sm tracking-widest shadow-neon active:scale-95">Save Vibe</button>
          </div>
        </div>
      )}
      ...
      {isEditingProfile && (
        <div className="fixed inset-0 bg-black/95 z-[400] flex items-center justify-center p-4" onClick={() => setIsEditingProfile(false)}>
           <div className="bg-[#1a0b2e] border-2 border-[#FF2E95] p-10 rounded-[50px] w-full max-w-sm shadow-2xl" onClick={e => e.stopPropagation()}>
              <div className="flex flex-col items-center gap-6 text-center">
                ...
                <button onClick={() => setIsEditingProfile(false)} className="w-full bg-[#FF2E95] text-white font-black py-4 rounded-2xl uppercase tracking-widest">Update</button>
              </div>
           </div>
        </div>
      )}

      <style>{`.shadow-neon { box-shadow: 0 0 20px rgba(209,255,75,0.3); } .custom-scrollbar::-webkit-scrollbar { width: 3px; } .custom-scrollbar::-webkit-scrollbar-thumb { background: rgba(255,255,255,0.1); border-radius: 10px; } .input-premium { background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.1); border-radius: 15px; padding: 10px 15px; font-weight: 700; color: white; width: 100%; outline: none; font-size: 11px; } .input-premium:focus { border-color: #D1FF4B; background: rgba(255,255,255,0.06); }`}</style>
    </div>
  )
}
export default App
